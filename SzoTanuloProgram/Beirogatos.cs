    ...
    public partial class Beirogatos : Form
    {
        ...
        private void _buttonNext_Click(object sender, EventArgs e)
        {
            if (_number == 1)  //MUTASD
            {
                ...
                string TextInput = _angolSzovegDoboz.Text;
                TextInput = Regex.Replace(TextInput, @"\s", "");

                string TextObjective = _angolSzovegDobozIros.Text;
                TextObjective = Regex.Replace(TextObjective, @"\s", "");

                Constants.Say(_angolSzovegDoboz.Text);

                //TUDTA
                if (TextInput == TextObjective)
                {
